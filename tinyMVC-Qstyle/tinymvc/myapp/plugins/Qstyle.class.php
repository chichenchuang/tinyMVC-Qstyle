<?php
/*
# @copyright: 分享工作室 Yuan 2016 11 01
# @filename; Qstyle.class.php
# @version: Qstyle 7.5.2;
*/
class Qstyle{
	public $templates_dir         = array('./tinymvc/myapp/views/');       //模板路径,支持数组叠代多层目录,最后面的优先搜索;
    public $templates_cache       = './tinymvc/myapp/cache_tpl/';            //缓存模板路径;
    ...
    protected $is_extract = 0;                      // 是否释放
    protected $templates_host  = '';
	protected $PHPnew = 'Qstyle 7.5.2';
	...
	public function display($PHPnew_file_name, $returnpath = false){
        ...
        $this->templates_cache_file[$PHPnew_file_name] = !empty($tplcache['cache']) ? $tplcache['cache'] : "";
        $this->templates_file[$PHPnew_file_name] = !empty($tplcache['tpl']) ? $tplcache['tpl'] : "";
    ...
    protected function preg__urlhost(){
        return 'https://'.$_SERVER['HTTP_HOST'].'::'.$_SERVER['SERVER_PORT'].dirname($_SERVER['PHP_SELF']);
    }
	...
	protected function __parse_html($template){
        ...
        $template = '<?php /* '.$this->templates_file[$this->templates_name].' */ if(is_object($this) === false){exit(\'Hacking!\');}else if(!$this->is_extract){$this->is_extract=1; extract($this->templates_assign);}?>'.$template;
